...
$koneksi = mysqli_connect("localhost","root","","ecom_store");
...
if (isset($_GET["action"])){
    if ($_GET["action"] == "delete"){
        ...
        $query = mysqli_query($koneksi, "INSERT INTO costumer_orders (order_date,due_amount) VALUES ('".date("Y-m-d")."','$total')");
        $id_trans = mysqli_insert_id($koneksi);

        foreach($_SESSION["cart"] as $key => $value){
            $id_prod = $value['product_id'];
            $qty = $value['item_quantity'];
            $sql = "INSERT INTO detail (id,order_id,product_id,quantity) VALUES ('$id', '$order_id', '$product_id', '$quantity')";
            $res = mysqli_query($koneksi, $sql); 
        }
        
        unset($_SESSION["cart"]);
        echo '<script>alert("Terima kasih sudah berbelanja!")</script>';
        echo "<script>window.location='cetak.php?id=".$order_id."'</script>";
